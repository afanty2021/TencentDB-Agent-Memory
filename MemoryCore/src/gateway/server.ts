...
export class TdaiGateway {
   ...
   *
   * Scope note (v3 port): this routing covers the v1-protocol endpoints
   * (recall / capture / search / session-end). The v3 surface (chat-memory,
   * knowledge, metadata) keeps its own instance/tenant resolution —
   * see PORT-NOTES.md.
   */
  ...
  private userCoreDataDir(uid: string): string {
    ...
    return join(this.config.data.baseDir, "users", uid);
  }

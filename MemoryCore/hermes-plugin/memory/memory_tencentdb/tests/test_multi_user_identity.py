...
def test_capture_snapshot_survives_current_user_overwrite(provider_with_fake_supervisor):
    ...
    fake.client.conversation_add.side_effect = _blocking_capture
    ...
    release_capture.set()
    assert _wait_until(
        lambda: fake.client.conversation_add.call_args is not None
        and fake.client.conversation_add.call_args.kwargs.get("user_id") == "alice"
    )

    assert fake.client.conversation_add.call_args.kwargs["user_id"] == "alice", (
...
def test_capture_uses_turn_author_over_current_user(provider_with_fake_supervisor):
    ...
    fake.client.conversation_add.side_effect = lambda **kw: capture_called.set()

    provider.on_turn_start(1, "hello", author_id="Alice")
    provider.sync_turn("u", "a", turn_author={"id": "Carol", "is_bot": False})

    assert capture_called.wait(timeout=2.0)
    assert fake.client.conversation_add.call_args.kwargs["user_id"] == "carol"
...
def test_capture_falls_back_to_static_user_id(provider_with_fake_supervisor):
    ...
    assert capture_called.wait(timeout=2.0)
    assert fake.client.conversation_add.call_args.kwargs["user_id"] == "test-user"
...
def test_capture_omits_user_id_when_chain_empty(provider_with_fake_supervisor):
    ...
    assert capture_called.wait(timeout=2.0)
    assert fake.client.conversation_add.call_args.kwargs["user_id"] == ""
...
def test_recall_prefers_current_user(provider_with_fake_supervisor):
    provider = provider_with_fake_supervisor
    fake = provider._fake
    # v3 prefetch fans out to atomic_search / core_read / scenario_ls.
    fake.client.atomic_search.return_value = {"data": {"items": []}}
    fake.client.core_read.return_value = {"data": {"items": []}}
    fake.client.scenario_ls.return_value = {"data": {"items": []}}

    provider.on_turn_start(1, "hello", author_id="Alice")
    provider.prefetch(query="hello")

    assert fake.client.atomic_search.call_args.kwargs["user_id"] == "alice"
...
def test_recall_falls_back_to_static_user_id_after_bot_turn(provider_with_fake_supervisor):
    ...
    assert fake.client.atomic_search.call_args.kwargs["user_id"] == "test-user"
...
def test_recall_omits_user_id_when_chain_empty(provider_with_fake_supervisor):
    ...
    assert fake.client.atomic_search.call_args.kwargs["user_id"] == ""
...
def test_conversation_search_tool_passes_user_id(provider_with_fake_supervisor):
    provider = provider_with_fake_supervisor
    fake = provider._fake
    fake.client.conversation_search.return_value = {"data": {"items": []}}

    provider.on_turn_start(1, "hello", author_id="Alice")
    out = provider.handle_tool_call("memory_tencentdb_conversation_search", {"query": "q"})

    assert "error" not in out.lower()
    assert fake.client.conversation_search.call_args.kwargs["user_id"] == "alice"


# ---------------------------------------------------------------------------
# end_session chain — v3 note: upstream's on_session_end/shutdown end_session
# calls are commented out (session lifecycle is owned by the v3 metadata
# layer), so the client-level chain assertions have no live call site here.
# The skipped tests are kept as the spec for re-enabling those call sites.
# ---------------------------------------------------------------------------


@pytest.mark.skip(reason="v3: provider-side end_session is disabled upstream; kept as spec")
...
# ---------------------------------------------------------------------------
# client: v3 search request bodies always carry the tenancy triple, with
# user_id resolved by the provider chain ("" → gateway default pool)
# ---------------------------------------------------------------------------
...
def test_atomic_search_body_includes_user_id(capture_post):
    client, calls = capture_post

    client.atomic_search(query="q", limit=3, type_filter="persona", user_id="alice")

    path, body = calls[0]
    assert path == "/v3/atomic/search"
    assert body["user_id"] == "alice"
    assert body["type"] == "persona"
    assert body["limit"] == 3


def test_atomic_search_body_defaults_user_id_when_absent(capture_post):
    client, calls = capture_post

    client.atomic_search(query="q")

    _, body = calls[0]
    assert body["user_id"] == "default"


def test_conversation_search_body_includes_user_id(capture_post):
    client, calls = capture_post

    client.conversation_search(query="q", session_id="sess", user_id="bob")

    path, body = calls[0]
    assert path == "/v3/conversation/search"
    assert body["user_id"] == "bob"
    assert body["session_id"] == "sess"


def test_conversation_search_body_defaults_user_id_when_absent(capture_post):
    client, calls = capture_post

    client.conversation_search(query="q")

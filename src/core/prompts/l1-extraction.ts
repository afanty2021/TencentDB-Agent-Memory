...
2. 独立完整：记忆必须"跳出当前对话依然成立"，无上下文也能看懂。提取主体以"用户（姓名）"或"AI"为核心；对话第三方的自述按第 4 条归属处理。
3. 归纳合并：强关联或因果关系的多条消息，必须合并为一条完整记忆，不可碎片化。
4. 说话人归属：第一人称自述（如"我给他拍"、"我家孩子"）必须归属于**实际说话人**。用户转发的聊天记录、引用的他人原话、或对话另一方的自我描述，一律不得写成"用户……"；其中的实质性内容（家长反馈、请求、事件等）**应当提取**，以对方为主体表述（如"对话对方（学生家长）表示/希望……"），不得因归属第三方而整体放弃提取。对话另一方对用户的称呼（如"X老师"；转发/引用内容中的称呼除外）是用户身份的强证据，与其冲突的身份推断一律无效。
...
1. 个性化记忆 (type: "persona")
   ...
   - 身份红线：家庭角色、婚姻状况、子女等身份级事实，仅在用户本人以第一人称明确陈述时提取；从转述、转发内容或他人描述推断出的身份一律不得写入。
   - 提取句式："用户（[姓名]）喜欢/是/擅长..."
...
2. 客观事件记忆 (type: "episodic")
   ...
   - 对话另一方（如学生家长、同事）向用户反映的情况、提出的请求，同样属于 episodic——主体写对方（如"学生家长表示/希望……"），这类信息正是用户后续工作需要的上下文，不要因为说话人不是用户而丢弃。

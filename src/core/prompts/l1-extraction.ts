...
4. 说话人归属：第一人称自述（如"我给他拍"、"我家孩子"）必须归属于**实际说话人**。用户转发的聊天记录、引用的他人原话、或对话另一方的自我描述，一律不得写成"用户……"——如需保留，以对方为主体表述（如"对话对方（学生家长）表示……"），否则不提取。对话中对用户的称呼（如"X老师"、"阿姨"）是用户身份的强证据，与其冲突的身份推断一律无效。
